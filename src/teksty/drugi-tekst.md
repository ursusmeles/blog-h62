---
title: "Tekst na próbę"
description: "To blog o sandboksie i grach fabularnych w ogóle."
lead: "Drugi tekst na blogu. Sprawdzamy jak działa post-grid. To będzie coś dłuższego, żebym sprawdził jak w takich sytuacjach zachowują się poszczególne komponenty."
og_image: "drugi.png"
image: "/img/post-alchemy.jpg"
category: "Kategoria"
date: 2026-02-20
---

A oto link. Napiszę więcej tekstu, żeby sprawdzić jak radzi sobie link z obniżonym podkreśleniem w przypadku kilku linijek. Ale wygląda to cyba dobrze, choć dla pewności sprawdzę to jeszcze.

## Czym jest Heliocantharus?
To blog o sandboksie i grach fabularnych w ogóle.

A oto link: [Przejdź dalej](#). Napiszę więcej tekstu, żeby sprawdzić jak radzi sobie link z obniżonym podkreśleniem w przypadku kilku linijek. Ale wygląda to cyba dobrze, choć dla pewności sprawdzę to jeszcze.

<a class="button">Kliknij mnie</a><a class="button" button-variant="secondary">Kliknij mnie</a><a class="button" button-variant="typographic">Kliknij mnie</a><a class="button" button-variant="fancy">Kliknij mnie</a>

---

### Heading poziomu trzeciego, który jest dodatkowo dłuższy
Trochę tekstu.
Jeszcze więcej tekstu. Zobaczymy, jak to się sprawdza.
### Kolejny heading trzeciego poziomu
Zobaczmy, jak to wygląda:
* Tekst
* Drugi
* Trzeci
#### To była lista, czas na heading poziomu 4

Napiszę więcej tekstu, żeby sprawdzić jak radzi sobie link z obniżonym podkreśleniem w przypadku kilku linijek. Ale wygląda to cyba dobrze, choć dla pewności sprawdzę to jeszcze.

Napiszę więcej tekstu, żeby sprawdzić jak radzi sobie link z obniżonym podkreśleniem w przypadku kilku linijek. Ale wygląda to cyba dobrze, choć dla pewności sprawdzę to jeszcze.

---

##### Wow, to jest H5
Napiszę więcej tekstu, żeby sprawdzić jak radzi sobie link z obniżonym podkreśleniem w przypadku kilku linijek. Ale wygląda to cyba dobrze, choć dla pewności sprawdzę to jeszcze.